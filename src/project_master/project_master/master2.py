...
class Master(Node):
    def __init__(self):
        ...
        self.cmd_vel_pub = self.create_publisher(TwistStamped, f'/robot2/project_controller/cmd_vel', 10)
        self.cmd_vel_pub_real = self.create_publisher(Twist, f'/robot1/project_controller/cmd_vel', 10)
        ...
        for robot_name in self.robot_names:
            ...
            # --- Obsługa dotarcia do DOCK ---
                ...
                if dist_to_target < arrival_threshold and is_stopped:
                    ...
                    if robot_name == "robot1":
                        stop_msg_real = Twist()
                        self.cmd_vel_pub_real.publish(stop_msg_real)
                    else:
    ...
    def obrot_180(self, robot_name):
        ...
        if abs(error) < 0.05:
            ...
            if robot_name == "robot1":
                cmd_real = Twist()
                cmd_real.linear.x = 0.0
                cmd_real.angular.z = angular_vel
                self.cmd_vel_pub_real.publish(cmd_real)
            else:
                cmd = TwistStamped()
